...
export class Clock {
 // hour: number;
  ...
  public plus(minutes: number): Clock {
    let reloj:Clock = new Clock(Math.floor((this.minute as number/ 60) % 24), this.minute as number+minutes)
  ...
  public equals(other: Clock): boolean {
    ...
    if (Math.floor((this.minute as number/ 60) % 24) == Math.floor((reloj.minute as number/ 60) % 24) && (this.minute as number) == reloj.minute) {
      return true;
    } else {
      return false;
    }
